...
| 26 | **Remaining gaps** | ⏳ Open | Monaco still ships/loads DOMPurify 3.2.7 (bundled copy and the jsdelivr CDN load in `@monaco-editor/react`) until monaco-editor releases a fix — consider `loader.config({ monaco })` to self-host. Kotlin repos no longer get contract tests (the old `ApiContractTest.kt` never compiled); re-add as a Kotlin-native generator. Pricing has no billing (copy says "free during beta"). |
