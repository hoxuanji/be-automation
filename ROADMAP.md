...
- **AWS, GCP, Azure, Kubernetes** — one-click via GitHub Actions: the pipeline (`runCloudDeployPipeline`) pushes the repo, stores the provider credentials as encrypted repository secrets (libsodium sealed box), dispatches the generated `.github/workflows/deploy.yml`, and streams the run. Credentials are verified in Settings → Integrations (`/api/{aws,gcp,azure,k8s}/verify`). Secret names come from `DEPLOY_SECRETS` (`src/lib/generators/deploy.ts`); `npm run test:deploy` checks Helios and the workflow agree.
...
| 24 | **Next 16 + generator parity** | ✅ Done | Next.js 16.3.6 (`src/middleware.ts` → `src/proxy.ts`, lint via eslint CLI); clears the postcss advisory. TS auth patterns use the provider's JWKS when one is configured (501 `handled_by_<provider>` for credential routes), self-issued HS256 only with `auth: none`; Fastify/Hono now enforce auth on protected routes. Litestar and Django emit custom endpoints + all 18 patterns. gRPC stream interceptors (Go, Python; grpc-js already covers streams). tRPC on Fastify, Hono and NestJS. |
| 25 | **Lint, advisories, Ktor tests** | ✅ Done | React hook rules `set-state-in-effect`, `purity`, `use-memo` re-enabled; 19 violations fixed with no disables. `npm audit` is 0 (js-yaml 4.3.2 and dompurify ^3.4.16 via `overrides`; in-range updates). Generated Ktor tests pass with no services (in-memory H2, test-signed RS256 tokens via `module(jwtVerifier)`); CI runs `gradle test` for Ktor. |
| 26 | **Self-hosted Monaco, Kotlin contract tests** | ✅ Done | Monaco is bundled from the pinned `monaco-editor` (0.55.1) via `src/lib/monaco.ts` (`loader.config({ monaco })` + module workers); no runtime CDN fetch. Kotlin contract tests are back, written natively for Ktor (reusing `TestSupport.kt`) and spring-kt (H2 `test` profile, `jwt()`); CI runs `gradle test` for all four Kotlin builds. spring-kt `SecurityConfig` now permits public stub routes (method-scoped; templated paths that could shadow a protected route stay protected). |
| 27 | **Everything wired + honest CI** | ✅ Done | Queues are real in every language (Kafka/Redpanda, RabbitMQ, NATS, SQS, BullMQ — non-Node BullMQ is a documented Redis-list fallback), with a worker entrypoint that docker compose runs. Rust, Java, Kotlin reach parity (tracing, rate limit, audit, Sentry/Datadog, Redis cache, queues). gRPC and GraphQL are DB-backed. Every deployment target gets a real `deploy.yml` job; AWS/GCP/Azure/K8s are one-click. CI runs with `pipefail` (earlier smoke results were not trustworthy), compiles 74 generated stacks (Java/Kotlin also run their tests), and an `e2e` job `docker compose up`s one stack per language and requires `/health?ready=1` + a running worker. |
| 28 | **Remaining gaps** | ⏳ Open | monaco-editor 0.55.1 bundles DOMPurify 3.2.7 (upstream). `dev`/`build` use `--webpack` (Turbopack drops Swagger UI's apidom registration). E2E checks readiness, not message round-trips (publish → worker consumes). Rust readiness doesn't ping the broker. Fastify crashes on a user-defined `GET /health` (duplicate route). TS `nats` package is deprecated upstream (`@nats-io/transport-node`). Builder doesn't yet warn that BullMQ outside Node and Vercel outside TS/FastAPI are limited. One-click AWS doesn't offer OIDC (`AWS_ROLE_ARN`). Older provider verify routes aren't rate-limited. Pricing has no billing. |
