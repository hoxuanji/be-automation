...
| 27 | **Remaining gaps** | ⏳ Open | monaco-editor 0.55.1 still bundles DOMPurify 3.2.7 internally (needs an upstream monaco release). `dev`/`build` use `--webpack`: Turbopack drops the `@swagger-api/apidom-*` `refractor/registration.mjs` side-effect modules (Swagger UI on /api-builder throws `refract is not a function`); CI's Production build checks the bundle, so re-test before switching back. Pricing has no billing (copy says "free during beta"). |
