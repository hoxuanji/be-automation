...
| 25 | **Remaining gaps** | ⏳ Open | Three React hook lint rules new in eslint-config-next 16 (`set-state-in-effect`, `purity`, `use-memo`) are off pending ~20 fixes. 5 prod npm advisories (js-yaml via swagger-ui-react, dompurify via monaco-editor, baseline-browser-mapping) — `npm audit fix` candidates. Generated Ktor tests don't init the DB and need `AUTH_*` env when auth is on. Pricing has no billing (copy says "free during beta"). |
