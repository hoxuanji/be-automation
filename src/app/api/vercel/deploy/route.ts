...
import { runVercelDeployPipeline } from "@/lib/deploy-pipeline";
...
export async function POST(req: NextRequest) {
  ...
  const vercelToken = allCreds.vercel?.token;
  if (!vercelToken) {
    return Response.json(
      {
        error: "vercel_token_missing",
        message: "No Vercel token saved.",
        hint: "Save a Vercel Personal Access Token under Settings → Integrations first.",
      },
      { status: 400 }
    );
  }
  ...
  try {
    const gen = runVercelDeployPipeline({
      githubToken,
      vercelToken,
      config,
      endpoints,
    ...
    console.error("[vercel/deploy] unexpected error:", err);
    return Response.json(
      {
        error: "vercel_error",
