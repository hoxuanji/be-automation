...
import { runFlyDeployPipeline } from "@/lib/deploy-pipeline";
...
export async function POST(req: NextRequest) {
  ...
  const flyToken = allCreds.fly?.token;
  if (!flyToken) {
    return Response.json(
      {
        error: "fly_token_missing",
        message: "No Fly API token saved.",
        hint: "Save a Fly Personal Access Token under Settings → Integrations first.",
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    console.error("[fly/deploy] unexpected error:", err);
    return Response.json(
      {
        error: "fly_error",
